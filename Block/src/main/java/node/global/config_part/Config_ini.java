package node.global.config_part;
...
import java.util.*;
public class Config_ini extends AbsSetting implements Map<String, String> {
     ...
     */
    public Config_ini(String path, boolean isUseVariable) {
     ...
     * @param isUseVariable 是否使用变量
     */
    public Config_ini(String path, Charset charset, boolean isUseVariable) {
     ...
     * @param isUseVariable 是否使用变量
     */
    public Config_ini(File configFile, Charset charset, boolean isUseVariable) {
     ...
     * @param isUseVariable 是否使用变量
     */
    public Config_ini(String path, Class<?> clazz, Charset charset, boolean isUseVariable) {
        Assert.notBlank(path, "Blank setting path !");
     ...
     * @param isUseVariable 是否使用变量
     */
    public Config_ini(URL url, Charset charset, boolean isUseVariable) {
        Assert.notNull(url, "Null setting url define!");
        this.init(new UrlResource(url), charset, isUseVariable);
    }
    // ------------------------------------------------------------------------------------- Constructor end

    private void init(Resource resource, Charset charset, boolean isUseVariable) {
        if (resource == null) {
    ...
    synchronized public boolean load() {
        if (null == this.loader) {
            loader = new Loader(this.groupedMap, this.charset, this.isUseVariable);
        }
        return loader.load(new UrlResource(this.settingUrl));
    }
    ...
    public String getAndRemoveStr(String... keys) {
        return (String) getAndRemove(keys);
    }
    ...
    /**
     * 获取group分组下所有配置键值对，组成新的{@link Config_ini}
     *
     * @param group 分组
     * @return {@link Config_ini}
     */
    public Config_ini getSetting(String group) {
        final Config_ini setting = new Config_ini();
     ...
     * @return this
     */
    public Config_ini putAll(String group, Map<? extends String, ? extends String> m) {
     ...
     * @return this
     */
    public Config_ini clear(String group) {
     ...
     * @since 3.3.1
     */
    public Config_ini set(String key, String value) {
        this.put(key, value);
        return this;
    }
     ...
     * @return 此key之前存在的值，如果没有返回null
     */
    public Config_ini set(String group, String key, String value) {
    ...
    public boolean equals(Object obj) {
        ...
        Config_ini other = (Config_ini) obj;
        if (charset == null) {
